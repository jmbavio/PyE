...
\addlegendentry{$\text{Bin}(n,0.5)$ estand.}
\end{axis}
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}{Ejemplo final: margen de error de una encuesta}
\begin{example}
Retomando la encuesta de la Clase 17 ($n=1500$, $p=0.48$ real), aproximar (en vez de acotar con Chebyshev) la probabilidad de que $\hat p$ se aparte de $0.48$ en más de $0.03$, usando el TCL.
\end{example}

\medskip
$\sigma_{\hat p} = \sqrt{0.0001664}\approx 0.01290$. Por el TCL, $\hat p \ \dot\sim\ N(0.48,\ 0.0001664)$:
\[
P(|\hat p-0.48|\geq 0.03) \approx 2\left[1-\Phi\left(\frac{0.03}{0.01290}\right)\right] = 2[1-\Phi(2.326)] \approx 2(1-0.9900) = 0.0200.
\]
\textbf{La probabilidad aproximada es solo $2\%$}, mucho más ajustada que el $18.5\%$ que daba Chebyshev en la Clase 17: al conocer que $\hat p$ es (aproximadamente) normal, la estimación de probabilidades es mucho más precisa que la cota universal de Chebyshev.
\end{frame}
