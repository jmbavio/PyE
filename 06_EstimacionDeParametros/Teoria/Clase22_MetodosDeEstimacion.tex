...
\begin{frame}{Interpretación gráfica de la verosimilitud}
\begin{block}{¿Qué significa "maximizar" $L(\theta)$?}
Para cada valor posible de $\theta$, $L(\theta)$ mide qué tan "compatible" es ese valor con los datos observados. El EMV es el punto más alto de esa curva.
\end{block}
\begin{center}
...
\end{tikzpicture}
\end{center}
\begin{alertblock}{Condición de máximo}
En el máximo, $\ell'(\widehat\theta)=0$ y $\ell''(\widehat\theta)<0$ (concavidad hacia abajo).
\end{alertblock}
\end{frame}
...
\begin{frame}{Comparación: mínimos cuadrados vs.\ máxima verosimilitud}
\begin{block}{Similitudes y diferencias}
\begin{itemize}
\item Ambos métodos son \textbf{criterios de optimización}: eligen el parámetro que "mejor explica" los datos, según un objetivo distinto.
\item MC minimiza distancias al cuadrado; no requiere especificar toda la distribución, solo la forma de la media.
\item EMV maximiza la probabilidad (o densidad) de observar los datos; requiere conocer la distribución completa.
\item Para el modelo normal con media constante, ambos métodos coinciden: $\widehat\mu_{MC} = \widehat\mu_{EMV} = \overline X$.
\end{itemize}
\end{block}
\begin{alertblock}{En la práctica}
El EMV es el método más usado en Estadística por sus buenas propiedades asintóticas (consistencia, eficiencia, normalidad); MC es central en regresión lineal (Unidad 8).
\end{alertblock}
\end{frame}
