...
\begin{frame}{Diagnóstico mediante el análisis de residuos}
Toda la inferencia desarrollada (pruebas $t$, $F$, intervalos) depende de los supuestos de normalidad, homocedasticidad e independencia de los errores. En la práctica se verifican de manera \textbf{informal} mediante gráficos de los residuos $e_i = y_i-\hat y_i$.

\begin{block}{Qué mirar en un gráfico de residuos vs.\ $\hat y_i$ (o vs.\ $x_i$)}
\begin{itemize}
\item \textbf{Nube sin patrón}, dispersa aleatoriamente alrededor de $0$: apoya los supuestos del modelo.
\item \textbf{Forma de embudo} (residuos que crecen o decrecen en magnitud): indica \textbf{heterocedasticidad} (viola $V(\varepsilon)=\sigma^2$ constante).
\item \textbf{Patrón curvo} (no aleatorio): sugiere que la relación no es lineal, o falta algún término en el modelo.
\item Puntos muy alejados del resto: posibles \textbf{valores atípicos} (outliers), a investigar.
\end{itemize}
\end{block}
...
\begin{frame}{Ejemplo: síntesis de los resultados}
\begin{table}[htbp]
\centering
\begin{tabular}{lc}
\toprule
Cantidad & Valor \\
\midrule
$\hat\beta_1$ (pendiente) & $3.893$ \\
IC 95\% para $\beta_1$ & $(3.535,\ 4.251)$ \\
$\hat\beta_0$ (ordenada) & $25.214$ \\
$CME$ & $0.899$ \\
